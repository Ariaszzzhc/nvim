...
local function theme_switcher(opts)
  ...
  local reload_theme = require("util.reload_theme")
  local list_theme = require("util.list_theme")
  ...
  if next(themes) ~= nil then
    ...
    local picker = pickers.new {
      ...
      attach_mappings = function()
        actions.select_default:replace( -- if a entry is selected, change current_theme to that
...
if present then
  return telescope.register_extension {exports = {themes = theme_switcher}}
else
  error "Cannot find telescope!"
end
